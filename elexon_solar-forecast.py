# # FINAL VERSION WITH PLOTLY SCATTER PLOTS AND DISTINCT COLORS
...
#     # Create a scatter plot for each process type using Plotly
#     fig = go.Figure()
...
#         # Add data to Plotly figure with markers and distinct colors
...
if start_datetime_utc < end_datetime_utc:
    ...
    # Create a line plot for each process type using Plotly
    fig = go.Figure()
    for i, (forecast, color) in enumerate(zip(forecasts, colors)):
        if forecast.empty:
            st.write(f"No data available for process type: {process_types[i]}")
        ...
        # Create a complete time range with the same timezone as the forecast data
        ...
        # Merge forecast data with the complete time range
        forecast = full_time_df.merge(forecast, on='start_time', how='left')

        # Add data to Plotly figure with lines and distinct colors
        fig.add_trace(go.Scatter(
            x=forecast["start_time"],
            y=forecast["quantity"],
            mode='lines',
